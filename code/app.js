...
//实现原理：
    ...
    //导入 className 切换类样式、eventlisteners 注册事件、style 行内样式、props 设置DOM元素的属性
    //当初始的 HTML 文档被完全加载和解析完成之后，使用 h函数 生成 vnode，然后 patch 到 id 为 container 的 div 上生成页面结构
    //排序按钮通过 eventlisteners 的 on 属性调用排序函数，点击按钮调用排序函数，然后重新渲染页面；class 判断当前的是以什么排序，对应的按钮显示为激活状态；style 设置整个列表的高度
    //在定义列表的行结构时，使用 props 模块设置 DOM 的 key 属性；通过 style 模块添加 delayed(进场样式) 和 remove(退场样式) 以及 opacity 和 transform; hook 属性中设置了 insert 属性，也就是在节点插入到 dom 中后，触发该回调，设置节点的高度
    //每一行的最后一列添加删除的图标，使用 eventlisteners 的 on 属性调用删除函数，点击删除按钮删除该行，然后重新渲染页面
    //添加按钮的回调函数就是随机生成一个数据对象，但是其 rank 属性是全局递增的。然后对新数据调用 h函数 生成新的 vnode，并且 patch 到页面上，重新渲染了页面
